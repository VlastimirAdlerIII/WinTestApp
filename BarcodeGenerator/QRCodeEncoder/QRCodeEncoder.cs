...
using SystemUtils = AutoCont.Barcodes.Codec.Util.SystemUtils;
using QRCodeUtility = AutoCont.Barcodes.Codec.Util.QRCodeUtility;
using System.Diagnostics;
//using AutoCont.Barcodes.Properties;

namespace AutoCont.Barcodes.Codec
{
   public class QRCodeEncoder
   {
      ...
      public virtual string calQRCodeBinary(byte[] qrcodeData)
      {
         bool[][] qrCodeArray = calQrcode(qrcodeData);
         StringBuilder sb = new StringBuilder();
         for (int i = 0; i < qrCodeArray.Length; i++)
         {
            for (int j = 0; j < qrCodeArray.Length; j++)
            {
               sb.Append(qrCodeArray[j][i] ? "1" : "0");
            }
            sb.Append("|");
         }
         return sb.ToString();
      ...
      public virtual bool[][] calQrcode(byte[] qrcodeData)
      {
         ...
         try
         {
            ...
            MemoryStream memoryStream = new MemoryStream(BarcodeGenerator.Resources.GetResource(fileName));
